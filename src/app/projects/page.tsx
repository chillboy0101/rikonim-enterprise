...
export default async function ProjectsPage() {
  ...
  if (page?.enabled && page?.sections?.length) {
    ...
    const intro = page.sections.find(
      (s): s is Extract<SanityPageSection, { _type: 'projectsIntroSection' }> =>
        s._type === 'projectsIntroSection'
    );
    return (
      <>
        {hero ? (
          <PageHero
            ...
            imageUrl={hero.imageUrl ?? 'https://images.pexels.com/photos/8961141/pexels-photo-8961141.jpeg?auto=compress&cs=tinysrgb&w=2400'}
                  ...
                  {idx % 3 === 0 ? (
                    <>
                      <div className="md:col-span-4">
                        ...
                        {p.location ? <p className="mt-2 text-sm text-brand-steel">{p.location}</p> : null}
                      </div>
