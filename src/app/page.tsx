...
export default async function HomePage() {
  const page = await getSanityPageByRoute('/');
  ...
  if (page?.enabled && page?.sections?.length) {
    ...
    const hero = page.sections.find(
      (s): s is Extract<SanityPageSection, { _type: 'homeHeroSection' }> => s._type === 'homeHeroSection'
    );
    const work = page.sections.find(
      (s): s is Extract<SanityPageSection, { _type: 'homeWorkSection' }> => s._type === 'homeWorkSection'
    );
    const people = page.sections.find(
      (s): s is Extract<SanityPageSection, { _type: 'homePeopleSection' }> => s._type === 'homePeopleSection'
    );
    const approach = page.sections.find(
      (s): s is Extract<SanityPageSection, { _type: 'homeApproachSection' }> => s._type === 'homeApproachSection'
    );
    const statement = page.sections.find(
      (s): s is Extract<SanityPageSection, { _type: 'homeStatementSection' }> => s._type === 'homeStatementSection'
    );
    const featuredProjects = page.sections.find(
      (s): s is Extract<SanityPageSection, { _type: 'homeFeaturedProjectsSection' }> =>
        s._type === 'homeFeaturedProjectsSection'
    );
    const visionMission = page.sections.find(
      (s): s is Extract<SanityPageSection, { _type: 'homeVisionMissionSection' }> =>
        s._type === 'homeVisionMissionSection'
    );
    const servicesTeaser = page.sections.find(
      (s): s is Extract<SanityPageSection, { _type: 'homeServicesTeaserSection' }> =>
        s._type === 'homeServicesTeaserSection'
    );

    return (
      <>
        <section id="home-hero" className="relative overflow-hidden pt-[76px] md:pt-[84px]">
          <video
            ...
            loop
            preload="metadata"
            poster={hero?.posterUrl ?? '/videos/rikonim-home-hero.jpg'}
          >
            <source src={hero?.videoUrl ?? site.heroVideos.home} type="video/mp4" />
          </video>
            ...
            <div className="grid min-h-[calc(100svh-76px)] gap-10 py-14 md:min-h-[calc(100svh-84px)] md:items-start md:pb-20 md:pt-16 lg:grid-cols-12 lg:items-center lg:py-20">
              <div className="lg:col-span-7">
                ...
                <h1 className="mt-4 text-balance text-5xl font-semibold tracking-tightest text-white md:text-6xl lg:text-7xl">
                  {hero?.heading ?? 'Building enduring infrastructure with discipline, integrity, and exceptional execution.'}
                </h1>
              </div>
          ...
          <Container>
            <div className="grid gap-10 py-14 md:min-h-[76vh] md:grid-cols-12 md:items-center md:py-20">
              <div className="md:col-span-5">
                <p className="text-sm font-semibold tracking-[0.16em] text-brand-steel">{work?.kicker ?? 'Our Work'}</p>
                <h2 className="text-balance text-4xl font-semibold tracking-tightest text-brand-ink md:text-5xl">
                  {work?.heading ?? 'Extraordinary teams building inspiring projects.'}
                </h2>
                ...
                <div className="mt-8 flex flex-wrap gap-4">
                  <Link
                    href={work?.primaryCtaHref ?? '/projects'}
                  ...
                  >
                    {work?.primaryCtaLabel ?? 'View projects'}
                  </Link>
                  <Link
                    href={work?.secondaryCtaHref ?? '/services'}
                  ...
                  >
                    {work?.secondaryCtaLabel ?? 'Explore Services'}
                  </Link>
                </div>
              </div>
                ...
                <div className="overflow-hidden rounded-3xl border border-brand-ink/10 bg-brand-mist shadow-[0_22px_70px_rgba(11,18,32,0.12)]">
                  <div className="h-[280px] sm:h-[380px] md:h-[540px]">
                    <video className="h-full w-full object-cover" autoPlay muted playsInline loop preload="metadata">
                      <source src={work?.videoUrl ?? '/videos/home-services-section.mp4'} type="video/mp4" />
                    </video>
                  </div>
                </div>
              </div>
        ...
        <section className="relative overflow-hidden">
          <video className="absolute inset-0 h-full w-full object-cover" autoPlay muted playsInline loop preload="metadata">
            <source src={people?.videoUrl ?? '/videos/home-company-section.mp4'} type="video/mp4" />
          </video>
          ...
          <Container className="relative">
            <div className="grid min-h-[72vh] gap-10 py-14 md:grid-cols-12 md:items-end md:py-20">
              <div className="md:col-span-6">
                <p className="text-sm font-semibold tracking-[0.16em] text-white/70">{people?.kicker ?? 'Our People'}</p>
                <h2 className="text-balance text-5xl font-semibold tracking-tightest text-white md:text-6xl">
                  {people?.heading ?? 'Defined by the quality of our people.'}
                </h2>
                <div className="mt-7">
                  <Link
                    href={people?.ctaHref ?? '/leadership'}
                  ...
                  >
                    {people?.ctaLabel ?? 'Meet leadership'}
                  </Link>
                </div>
              </div>
              <div className="md:col-span-6">
        ...
        <section className="bg-brand-ink">
          <Container>
            <div className="grid gap-10 py-14 md:grid-cols-12 md:items-center md:py-20">
              <div className="md:col-span-5">
                <p className="text-sm font-semibold tracking-[0.16em] text-white/70">{approach?.kicker ?? 'Our Approach'}</p>
                <h2 className="mt-4 text-balance text-4xl font-semibold tracking-tightest text-white md:text-5xl">
                  {approach?.heading ?? 'Plan precisely. Build safely. Deliver with integrity.'}
                </h2>
              ...
              <div className="md:col-span-7">
                <div className="overflow-hidden rounded-3xl border border-white/10 bg-black shadow-[0_22px_70px_rgba(0,0,0,0.35)]">
                  <div className="h-[280px] sm:h-[380px] md:h-[520px]">
                    <video className="h-full w-full object-cover" autoPlay muted playsInline loop preload="metadata">
                      <source src={approach?.videoUrl ?? '/videos/home-approach-section.mp4'} type="video/mp4" />
                    </video>
                  </div>
                </div>
              </div>
            ...
            <div className="grid gap-10 py-14 md:grid-cols-12 md:py-20">
              ...
              <div className="md:col-span-6">
                <h2 className="text-center text-4xl font-semibold tracking-tightest text-brand-ink md:text-5xl">
                  {statement?.heading ?? 'Building tomorrow, together.'}
                </h2>
          ...
          <Container>
            <div className="grid gap-10 md:grid-cols-12 md:items-start">
              <div className="md:col-span-5">
                ...
                <div className="mt-6">
                  <Link
                    href={featuredProjects?.ctaHref ?? '/projects'}
                  ...
                  >
                    ...
                    <span>{featuredProjects?.ctaLabel ?? 'Explore more projects'}</span>
                  </Link>
                </div>
              </div>
              <div className="md:col-span-7">
                  ...
                  {featured.map((p) => (
                    ...
                    >
                      ...
                      <div className="relative flex min-h-[260px] flex-col justify-end p-5">
                        ...
                        <p className="mt-2 text-lg font-semibold leading-tight tracking-tightest text-white">{p.title}</p>
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            <div className="grid gap-10 md:grid-cols-12">
              <div className="md:col-span-5">
                <h2 className="text-2xl font-semibold tracking-tightest text-brand-ink md:text-3xl">
                  {visionMission?.heading ?? 'Vision & Mission'}
                </h2>
                <p className="mt-4 text-base leading-relaxed text-brand-steel">{visionMission?.vision ?? site.vision}</p>
              </div>
              <div className="md:col-span-7">
                <div className="border-l border-brand-ink/10 pl-6">
                  <p className="text-sm font-semibold tracking-[0.14em] text-brand-steel">
                    {visionMission?.commitmentTitle ?? 'What we commit to'}
                  </p>
                  <ul className="mt-5 space-y-4">
                    {(visionMission?.missionBullets ?? site.missionBullets).map((b) => (
        ...
        <Section className="bg-brand-mist">
          <Container>
            <div className="grid gap-10 md:grid-cols-12 md:items-center">
              <div className="md:col-span-5">
                <h2 className="text-2xl font-semibold tracking-tightest text-brand-ink md:text-3xl">
                  {servicesTeaser?.heading ?? 'Premium delivery, end to end.'}
                </h2>
                <p className="mt-4 text-base leading-relaxed text-brand-steel">
                  {servicesTeaser?.paragraph ??
              ...
              <div className="md:col-span-7">
                <div className="grid gap-6 sm:grid-cols-2">
                  {(servicesTeaser?.services ??
                    ...
                    <div key={s._key ?? s.title ?? ''} className="border-t border-brand-ink/10 pt-5">
                      <p className="text-sm font-semibold text-brand-ink">{s.title}</p>
                      <p className="mt-2 text-sm leading-relaxed text-brand-steel">
                        {s.body ?? 'Precision planning, transparent coordination, and workmanship built for longevity.'}
                      </p>
                    </div>
                  ))}
                </div>
